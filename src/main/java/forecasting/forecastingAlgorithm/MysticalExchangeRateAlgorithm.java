...
@Slf4j
public class MysticalExchangeRateAlgorithm extends AbstractExchangeRateAlgorithm
    ...
    protected CurrencyRate findCurrencyRateForDate(List<CurrencyRate> exchangeRates, Date date) {
        List<CurrencyRate> matchingRates = new ArrayList<>();
        ...
        if (matchingRates.isEmpty()) {
            ...
            int randomIndex = random.nextInt(matchingRates.size());
            return matchingRates.get(randomIndex);
        }
    }
}
